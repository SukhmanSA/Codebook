...
import { createOrders, getUser } from "../../../services/dataService"


export const Checkout = ({useCheckout,setUseCheckout}) =>{
    ...
    async function handleOrderSubmit(event){
        ...
        try{
        const data = await createOrders(total,cartList,user)
        clearCart()
    ...
    return (
        <section>
            ...
            <div id="authentication-modal" tabIndex="-1" className="-mt-2 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full justify-center items-center flex" aria-modal="true" role="dialog" >
                <div className="relative p-4 w-full max-w-md h-full md:h-auto overflow-y-auto">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button onClick={()=> setUseCheckout(!useCheckout)} type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="authentication-modal" >
